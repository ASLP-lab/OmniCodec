...
class ResidualVectorQuantizer(BaseQuantizer):
    ...
    def forward(self, x: torch.Tensor, frame_rate: int, layers: tp.List[int] = []):
        """
        Args:
        ...
        if self.training and self.q_dropout:
            n_q = self.rng_dropout.randint(8, self.n_q) # 8-16随机一个数字，后面的码本不量化，迫使前面几层码本量化多的信息
        bw_per_q = math.log2(self.bins) * frame_rate / 1000
        quantized, codes, commit_loss, metrics, q_layers = self.vq(x, n_q=n_q, layers=layers)
        ...
        return QuantizedResult(quantized, codes, bw, penalty=torch.mean(commit_loss), metrics=metrics, q_layers=q_layers)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rvq_first = ResidualVectorQuantizer(
            n_q=n_q_semantic, force_projection=True, q_dropout=False, no_quantization_rate=0.4, **kwargs
        )
        self.rvq_rest = ResidualVectorQuantizer(
            n_q=n_q - n_q_semantic,
            codebook_offset=1,
            force_projection=True,
            no_quantization_rate=0.2,
            q_dropout=q_dropout,
            **kwargs,
        )
    ...
    def forward(self, x: torch.Tensor, frame_rate: int):
        ...
        Returns:
            QuantizedResult: Quantized result with the following attributes:
                ...
                - `metrics` (dict): RVQ metrics, in particular rate of dead code replacement, and entropy.
        """
        semantic_result = self.rvq_first(x, frame_rate, [0])
        ...
        return QuantizedResult(
            ...
            q_layers=semantic_result.q_layers
        )
